...
All post-authentication traffic is protected by authenticated encryption. Each packet includes authenticated continuity metadata: a global counter plus a strictly increasing counter scoped to reliable session data, signaling, or unordered replaceable input. The receiver rejects duplicates or regressions inside each scope and transport epoch. Separate scopes prevent legitimate cross-channel reordering from invalidating keyboard input while preserving replay protection for every stream.
...
`desktop.preLogon` (default **true**) is the same trade made one step earlier. While nobody is logged on there is no `shell.user` to run as, so the launcher runs the agent as SYSTEM in the console session; that is what keeps a machine which rebooted unattended reachable at all, and an operator who can reach its logon screen can sign in to it. The exposure is bounded on purpose: such an agent serves `desktop` sessions only - never a terminal, never the file manager - it is retired the moment anyone logs on, and the flag that puts it in that mode is refused by any process that is not actually LocalSystem. `"desktop": { "preLogon": false }` gives up remote recovery in exchange for a machine that is simply dark until someone signs in at the keyboard. See [Reaching a machine before anyone logs on](windows.md#reaching-a-machine-before-anyone-logs-on).
