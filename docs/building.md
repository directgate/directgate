...
Desktop **system-audio** streaming is likewise runtime-only and optional. The agent loads `libopus` for encoding (`DIRECTGATE_OPUS_LIB` overrides the search) and, on Linux, `libpulse` to capture the PulseAudio / PipeWire default-sink monitor (`DIRECTGATE_AUDIO_SOURCE` overrides the source). Neither is a build dependency, and when either is missing the session still streams video - audio just reports `unavailable`. Most desktops already ship both; on a headless host install your distribution's `libopus` and `pulseaudio-libs` / `libpulse` packages to enable it.
