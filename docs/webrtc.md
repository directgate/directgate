...
The source is always the default output device's loopback, never a microphone, and every platform feeds the same encode/RTP path. Linux captures the PulseAudio / PipeWire default-sink monitor (`libpulse` dlopen'd, the `@DEFAULT_MONITOR@` special device, or the `DIRECTGATE_AUDIO_SOURCE` override); Windows captures the default render endpoint via WASAPI loopback on a dedicated MTA capture thread (resampled to 48 kHz); macOS captures the system mix via ScreenCaptureKit (`SCStreamConfiguration.capturesAudio`, macOS 13+, with `excludesCurrentProcessAudio` so the agent never records itself). The Opus encoder is `libopus`, dlopen'd at session start (`DIRECTGATE_OPUS_LIB` overrides the search). When the encoder or the capture source is unavailable, audio reports `audio: "unavailable"` with an `audioReason` and the video stream is untouched.
