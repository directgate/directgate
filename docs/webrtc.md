[Back to README](../README.md)

# WebRTC P2P & connectivity

## How it works

After the client successfully authenticates with the agent, it automatically initiates WebRTC peer-to-peer negotiation:

1. **Client creates a PeerConnection** and a data channel named `"directgate"`
2. libdatachannel **auto-generates an SDP offer** when the data channel is created
3. The offer is **relayed to the agent** (wrapped in a `webrtc/offer` message)
4. The agent **creates an answer** and sends it back via `webrtc/answer`
5. ICE candidates are exchanged via `webrtc/ice` messages through the relay
6. Once both sides complete ICE negotiation, the **data channel opens**
7. From this point on, all terminal I/O and file transfers flow **directly between peers**
8. ICE negotiation happens only after authentication, and all traffic remains end-to-end encrypted

## ICE/TURN server configuration

For NAT traversal behind symmetric NATs you typically need a TURN server. **By default, DirectGate's own managed TURN servers are used** - there is nothing to set up, and it works out of the box (subject to a fair-use quota on your account).

If you would rather **opt out of that quota** and use your own infrastructure, add your TURN servers in your account settings on [directgate.io](https://directgate.io) - you still do not edit anything on the agent. The API delivers your configured ICE servers to the agent inside the enrollment response, and refreshes them on every token refresh, so changes you make in the dashboard are picked up automatically the next time the agent connects.

The agent selects ICE servers in this order:

1. **ICE servers delivered by the API** - by default these are DirectGate's managed TURN servers; if you add your own in settings, those are sent instead.
2. **A local `iceServers` array** in the agent config - a manual override, useful for debugging:

   ```json
   {
     "iceServers": [
       "stun:stun.cloudflare.com:3478",
       "turn:username:password@turn.example.com:3478"
     ]
   }
   ```

3. **Built-in defaults** (`stun:stun.cloudflare.com:3478` and `stun:stun.l.google.com:19302`) when neither of the above is present.

TURN server URLs follow the format `turn:user:pass@agent:port`. Up to **8** ICE servers can be used.

## Fallback

The connection degrades gracefully through three tiers, and the client always shows which one is active:

1. **Direct P2P** - a direct peer-to-peer WebRTC data channel. The client shows the **P2P** icon.
2. **TURN** - if a direct path cannot be established (e.g. symmetric NAT), WebRTC relays through a TURN server. It is still an end-to-end-encrypted WebRTC data channel, just routed via TURN; the client replaces the P2P icon with a **TURN** icon.
3. **WebSocket relay** - if WebRTC cannot connect at all (even via TURN), the session transparently falls back to the WebSocket relay path through directgate.io, and the icon disappears. End-to-end encryption is preserved, and the relay still cannot decrypt.

The terminal session stays fully functional on every tier, and traffic remains end-to-end encrypted regardless of which one is in use - so you always know whether the connection is direct, TURN-relayed, or on the WebSocket relay.

## Desktop video track

Desktop sessions reuse the same authenticated WebRTC connection but add a send-only H.264 **media track** (host -> browser) on top of the `directgate` DataChannel. Negotiation happens only after SRP/key-auth succeeds and the browser starts a `desktop` session. The browser offer carries:

- the existing ordered `directgate` DataChannel (encrypted DirectGate binary protocol)
- a recv-only video transceiver that advertises H.264

When the active session mode is `desktop`, the agent answers with an H.264 send-only track. The capture backend emits Annex-B H.264 access units (ScreenCaptureKit + VideoToolbox on macOS; X11 XShm capture + a runtime-loaded [Cisco OpenH264](https://github.com/cisco/openh264) encoder on Linux; DXGI Desktop Duplication + a Media Foundation encoder on Windows), and the agent packetizes them as RTP and sends them through libdatachannel's Track API (`rtcAddTrackEx` / `rtcSendMessage`). The browser renders the remote `MediaStreamTrack` in a `<video>` element. On Linux the OpenH264 library is dlopen'd at session start (`DIRECTGATE_OPENH264_LIB` overrides the search path); when it is missing - or the X11 pixel format is unsupported - the agent demotes the session to the raw-RGBA pipeline and reports the reason in the desktop status `fallbackReason` field. On Windows the agent prefers the GPU vendor's hardware H.264 encoder (Quick Sync / NVENC / AMF, whichever MFT the driver registered) and falls back to the Microsoft software encoder; `mfplat.dll` is loaded at runtime, so N editions without the Media Feature Pack demote to raw RGBA the same way (see [Desktop streaming on Windows](windows.md#desktop-streaming)).

Xlib's default error handlers are replaced before the first call that can fail. That matters more than it sounds: Xlib answers a protocol error by calling `exit()`, and the ordinary act of unplugging a monitor or changing its resolution races the geometry the pipeline captured, so the next grab against the now-invalid rectangle returns BadMatch and would take the whole agent down - every session, PTY and file transfer with it. Protocol errors are recoverable (the failed grab is already handled as a capture failure), so they are logged, rate-limited and ignored.

On Linux the agent encodes on the **GPU** when it can. At session start it probes `h264_nvenc` (NVIDIA) -> `h264_vaapi` (Intel and AMD through libva) -> `h264_qsv` -> `h264_amf` -> `h264_v4l2m2m`, taking the first that actually opens; for the surface-based encoders it also walks `/dev/dri/renderD*` rather than trusting the default node, because a hybrid-graphics box routinely has one node that only decodes and another that encodes. Anything that fails - no libavcodec, no GPU, a codec that will not initialise, or a GPU that starts rejecting frames mid-session - falls back to the OpenH264 software encoder, so desktop mode never depends on hardware being present. That last case needs a *sustained* failure (30 consecutive frames): GPU encoders hiccup transiently when the surface pool drains or the driver is briefly busy, and dropping to software on the first error would cost the session its picture quality for good, since the fallback is latched until the pipeline is rebuilt. libavcodec is dlopen'd exactly like OpenH264 and libopus and is never linked, so it adds no package dependency; the loader additionally refuses a runtime libavcodec whose major soname differs from the build's, since the struct layouts come from the build-time headers. `DIRECTGATE_HWENC=0` forces the software encoder and `DIRECTGATE_HWENC_ENCODER=h264_vaapi` pins a specific one. On a Wayland session whose encoder is VAAPI the frame need not reach the CPU at all: the compositor is offered the choice of exporting its buffers, and one that accepts is imported straight into the encoder with the colour conversion and the resize done by the GPU's post-processor (`DIRECTGATE_HWENC_ZEROCOPY=0` turns the offer off; see [Desktop streaming on Linux](linux.md#wayland-sessions)). The pipeline start log reports which was chosen, e.g. `encoder(hardware: h264_nvenc (NVIDIA NVENC H.264 encoder))`. Quality on the GPU path comes from the encoder **preset**, and the bitrate is held to the target exactly. NVENC runs `p4` rather than the fastest `p1`: on a 720p screen-content clip `p1` measured 0.95 SSIM at 8 Mbps and barely improved with bitrate at all (0.81 at 1 Mbps, still only 0.85 at 4 Mbps), which is what made every adaptive-bitrate reduction look catastrophic; `p4` scores 0.99 at 8 Mbps and 0.96 at 3 Mbps for an encode cost of ~0.3 ms more per frame on dedicated silicon.

The preset bitrates name a single figure chosen for a roughly 1080p encode, and it is **restated for the size actually being encoded**: detail per pixel is what the viewer perceives, so the same 8 Mbps spread over a superwide monitor is a fraction of the quality. Measured on 30 fps screen content, 1600x900 at 8 Mbps (0.185 bpp) scores 0.997 SSIM while 3440x1440 at the same 8 Mbps (0.054 bpp) scores 0.952 and text stops being readable - which is why streaming such a monitor at its native size looked broken until the viewer picked a window-sized stream. The scaling only ever goes upwards, so anything at or below 1080p keeps the preset's figure untouched, and is capped at 2x so an oversized monitor cannot ask for a rate no link would carry (3440x1440 lands on 16 Mbps, 0.983 SSIM). The adaptive controller governs from there as usual.

Rate control stays **CBR with peak == average and a ~250 ms buffer**, deliberately *unlike* the software encoder's 1.5x `iMaxBitrate` headroom. Giving a GPU encoder that headroom measured 16-28% over target on NVENC, and the rate the adaptive controller picked is precisely the rate the link was measured to carry - exceeding it does not look like lower quality, it looks like the picture staying sharp while falling seconds behind, because the excess just queues. The headroom was worth ~0.002 SSIM next to raising the preset, so accuracy wins.

Note that GPU encoders cannot be retargeted mid-stream through libavcodec, so an adaptive-bitrate step re-opens the encoder; that is coalesced to changes of at least 20% and at most one every three seconds, because each rebuild costs one IDR. The step that restores the preset's full rate is exempt from the 20% rule - the controller recovers in ~10% increments, so its last step is always below the threshold, and skipping it would leave the encoder parked just under full quality until something rebuilt the pipeline.

On every platform, capture and encode run on a **dedicated thread** and publish each finished access unit into a single-slot mailbox; the agent event loop drains that mailbox and does the RTP send. This keeps encode time off the loop that also injects the viewer's input and services the WebRTC queue, so input latency does not track encoder cost. The producer skips a capture entirely while the mailbox is still occupied (nothing enters the encoder, so its reference chain stays intact and no keyframe is needed on resume), and the consumer discards a frame that sat in the mailbox for more than three frame periods, asking for an IDR to re-anchor the decoder. Frames are timestamped when they are *published*, not when they were captured, so a host whose encoder is slower than the drop threshold still streams instead of discarding everything it produces.

The media track is tuned for interactive latency rather than smooth playback. The agent chains an RTCP NACK responder onto the track, so packet loss is repaired by retransmission instead of escalating to a PLI and a full
keyframe; GOPs are long (10s) because IDR frames are ordered on demand via PLI / `request-keyframe` anyway. An adaptive bitrate controller (`DirectGate_Desktop_AbrStep`) consumes the browser's RTCP receiver reports (fraction lost) - and transport backpressure on the DataChannel fallback - stepping the encoder rate down 25% on congestion and recovering toward the preset target after ~5 clean seconds (the live rate is reported as `bitrateKbps` in the desktop status). Congestion is declared only after **two consecutive** lossy reports: reports arrive about once a second while the controller runs once per frame, and a single lossy report is repaired by the NACK responder without any rate change. Treating one report as congestion makes the controller a one-way ratchet - the down-step fires every 2s while recovery needs 5 unbroken clean seconds, so a link that hiccups occasionally walks to the floor and stays there, which looks like a permanently bad picture on a link that can carry the full preset. The browser side requests a zero-length jitter buffer (`jitterBufferTarget`) on the video receiver.

The pipeline degrades in this order, and the desktop status payload reports which one is active:

1. `webrtc-video` - preferred H.264 RTP over the WebRTC media track, encrypted by DTLS-SRTP
2. `h264-datachannel` - `desktop-frame-encoded` H.264 chunks over the AES-SIV-encrypted DataChannel
3. `raw-rgba` - `desktop-frame-chunk` raw frames (debug / legacy fallback)

Desktop input, control, status, terminal, file-manager, and file-transfer messages all stay on the encrypted DirectGate protocol over the DataChannel. TURN relays forward only encrypted WebRTC media/DataChannel packets - they never see plaintext desktop video or control messages.

Desktop status is a `type: "data"` packet with `payloadType: "desktop-status"` and a JSON payload; key fields are `status`, `pipeline`
(`webrtc-video` / `h264-datachannel` / `raw-rgba`), `codec`, `preset`
(`quality` / `balanced` / `low-latency`), `fps`, `bitrateKbps`, `transport`
(`dtls-srtp` for the media track, `aes-siv-datachannel` for DataChannel fallbacks), an optional `fallbackReason`, and the audio fields `audio`
(`off` / `capturing` / `unavailable`) with an optional `audioReason`.

## Desktop audio track

Desktop sessions can also carry the host's **system output audio** as a second send-only **Opus media track** on the same peer connection. It is strictly additive to the video track and shares its security and latency model: the same DTLS-SRTP transport (TURN relays still see only ciphertext), the same manual RTP packetization (`rtcAddTrackEx` with `RTC_CODEC_OPUS` / `rtcSendMessage`), and the same `msid` (`directgate-desktop`) so the browser groups audio and video into one `MediaStream` and plays both from the `<video>` element.

The browser offer adds a **recv-only Opus audio transceiver** alongside the video one; when the session mode is `desktop` the agent answers with a matching send-only Opus track. Audio and video are independent - a missing or unopened audio track never blocks the answer, the video pipeline, or the background TURN->P2P upgrade (audio rides through promotion but never gates it).

Audio is **opt-in**. The track is negotiated up front so the viewer can unmute instantly, but the host does not capture anything until the browser sends a `desktop-control` `{ "action": "audio", "enabled": true }` message; `enabled: false` stops capture. Capture runs on a dedicated thread (48 kHz stereo, 20 ms frames, ~128 kbps Opus with in-band FEC) and the encoded frames are drained onto the main loop and sent over the track, so audio never gates the video encode.

The source is always the default output device's loopback, never a microphone, and every platform feeds the same encode/RTP path. Linux captures the PulseAudio / PipeWire default-sink monitor (`libpulse` dlopen'd, the `@DEFAULT_MONITOR@` special device, or the `DIRECTGATE_AUDIO_SOURCE` override); Windows captures the default render endpoint via WASAPI loopback on a dedicated MTA capture thread (resampled to 48 kHz); macOS captures the system mix via ScreenCaptureKit (`SCStreamConfiguration.capturesAudio`, macOS 13+, with `excludesCurrentProcessAudio` so the agent never records itself). The Opus encoder is `libopus`: dlopen'd at session start on Linux/macOS (`DIRECTGATE_OPUS_LIB` overrides the search) and linked statically into the exe on Windows (which has no package manager), so it is always present there. When the encoder or the capture source is unavailable, audio reports `audio: "unavailable"` with an `audioReason` and the video stream is untouched.

## See also

- [Architecture](architecture.md) - how the agent, relay, and client fit together
- [Configuration](configuration.md) - the `iceServers` config field
