/* A local fake server accepts PulseAudio's connection but never answers. This
 * tests the real dynamically loaded API and timeout without capturing audio. */
#include <stdio.h>
#include <sys/un.h>
...
{
    char error[256];
    if (DirectGate_Audio_LoadPulse(error, sizeof(error)) != XSTDOK) return 77;
    ...
    pid_t child = fork();
    CHECK(child >= 0, "server child");
    if (!child)
    {
        alarm(7);
        int peer = accept(fd, NULL, NULL);
        if (peer >= 0) { sleep(6); close(peer); }
        close(fd);
        _exit(0);
    }
    close(fd);
    ...
    setenv("DIRECTGATE_AUDIO_SERVER", server, 1);
    ...
    CHECK(elapsed >= 2500000 && elapsed < 5000000, "real PulseAudio handshake has a deadline");
