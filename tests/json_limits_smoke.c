...
#define CHECK(c, msg) do { if (!(c)) { \
    fprintf(stderr, "json_limits_smoke: %s\n", msg); return 1; } } while (0)
...
int main(void)
{
    ...
    CHECK(json.nError == XJSON_ERR_DEPTH && json.pRootObj == NULL,
        "depth failure frees partial tree");
    XJSON_Destroy(&json);
    ...
    data[pos++] = '{';
    for (size_t i = 0; i < count; i++)
        pos += (size_t)sprintf(data + pos, "\"key%zu\":0%c", i, i + 1 < count ? ',' : '}');
