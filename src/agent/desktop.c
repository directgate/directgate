...
XSTATUS DirectGate_Desktop_EnumerateMonitors(directgate_desktop_t *pDesktop)
{
    ...
    if (nStatus == XSTDOK) ++pDesktop->nPreviewGeneration;
...
XSTATUS DirectGate_Desktop_StartStream(directgate_desktop_t *pDesktop)
{
    ...
    if (nStatus == XSTDOK) pDesktop->bStreaming = XTRUE;
...
XSTATUS DirectGate_Desktop_StopStream(directgate_desktop_t *pDesktop)
{
    ...
    if (nStatus == XSTDOK) pDesktop->bStreaming = XFALSE;
                                                  ...
                                                  directgate_desktop_quality_t ePreset)
{
    ...
    if (nStatus == XSTDOK) pDesktop->ePreset = ePreset;
                                             ...
                                             const char *pMonitorId)
{
    ...
    if (nStatus == XSTDOK) xstrncpy(pDesktop->sSelectedMonitorId, sizeof(pDesktop->sSelectedMonitorId), pMonitorId);
                                  ...
                                  const char *pFallbackReason)
{
    ...
    XJSON_AddStrIfUsed(pRoot, "fallbackReason", pReason);
    XJSON_AddStrIfUsed(pRoot, "selectedMonitorId", pDesktop->sSelectedMonitorId);
...
static int DirectGate_Desktop_SendPreview(directgate_session_t *pSession, const char *pMonitorId)
{
    ...
    XJSON_AddString(pHeader, "format", preview.eFormat == DIRECTGATE_DESKTOP_PREVIEW_PNG ? "png" : "jpeg");
